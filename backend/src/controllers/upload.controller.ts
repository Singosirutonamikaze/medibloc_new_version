...
/**
 * Extrait un paramètre de route en tant que nombre entier.
 * req.params peut typer une valeur comme string | string[] (Express 5),
 * donc on rejette explicitement le cas tableau avant de parser.
 */
...
export class UploadController {
  ...
  public uploadAvatar = async (
    ...
    let status = 200;
    let response: ApiResponse<{ avatarUrl: string } | null> = {
      success: true,
      data: null,
    };
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("File too large")) {
        ...
        response = {
          success: false,
          error: "Fichier trop volumineux. Maximum autorisé : 2 Mo",
        };
      } else {
        status = 500;
        response = {
